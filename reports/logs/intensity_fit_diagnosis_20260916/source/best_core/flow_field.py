...
class FlowField(nn.Module):
    ...
    @torch.no_grad()
    def reset_mlp_parameters(self, *, seed: int) -> None:
        """Initialize the shared Torch MLP independently of field construction.

        Scene-field adapters allocate different numbers of Torch parameters
        before constructing ``FlowField``.  A private RNG scope prevents that
        implementation detail from changing the initialization of the flow
        module shared by paired representation experiments.
        """
        with torch.random.fork_rng(devices=[]):
            ...
            torch.nn.init.normal_(self.mlp[-1].weight, 0, 0.001)
