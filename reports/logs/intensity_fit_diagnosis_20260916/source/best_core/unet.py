...
from best_core.visibility_residual_basis import (
    VISIBILITY_RESIDUAL_NONE,
    build_visibility_residual_basis,
)
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        out_channels=1,
        *,
        visibility_residual_basis=VISIBILITY_RESIDUAL_NONE,
        ...
        self.residual_basis = build_visibility_residual_basis(
            visibility_residual_basis,
            depth_scale=visibility_depth_scale,
        )
    ...
    def forward(self, input):
        logits = self.forward_logits(input)
        if self.residual_basis is not None:
            logits = self.residual_basis(logits, input)
