...
from model.stgc_coordinates import field_flow_in_sensor
...
class Trainer(object):
    def __init__(
        ...
        self.name = name
        self.opt = opt
        self.field_training = not (opt.test or opt.test_eval or opt.refine)
        self.mute = mute
        ...
        if model_flow is not None:
            model_flow.to(self.device).eval().requires_grad_(False)
            with torch.random.fork_rng():
                torch.manual_seed(self.opt.seed)
                self.net_vlad = NetVLADLoupe(
                    feature_size=128, max_samples=self.opt.num_rays_lidar,
                    cluster_size=64, output_dim=128)
            self.net_vlad.to(self.device).eval().requires_grad_(False)
        ...
        self.scaler = torch.cuda.amp.GradScaler(enabled=self.fp16, init_scale=self.opt.amp_init_scale)
        ...
        if self.workspace is not None:
            if self.use_checkpoint == "scratch":
                self.log("[INFO] Fresh optimizer/counters; model weights are already initialized.")
    ...
    def train_step(self, data):
        ...
        # main loss
        loss = lidar_loss.sum()
        if getattr(self.opt, "alpha_intensity_gradient", 0.0) > 0:
            from best_core.appearance_objectives import intensity_gradient_loss
            gradient = intensity_gradient_loss(
                outputs_lidar["image_lidar"][..., 1], images_lidar[..., 1], gt_raydrop,
                images_lidar[..., 2] / self.opt.scale, self.opt.patch_size_lidar)
            loss = loss + self.opt.alpha_i * self.opt.alpha_intensity_gradient * gt_raydrop.numel() * gradient
        if self.opt.field_backend == "best":
            loss = loss + self.opt.alpha_spline_knots * self.model.temporal_regularization_loss()
            loss = loss + self.opt.alpha_high_order_regularization * self.model.high_order_regularization_loss()

        # additional CD Loss
        with torch.cuda.amp.autocast(enabled=False):
            rotation = data["poses_lidar"][:, :3, :3].float()
            pred_world = rays_o_lidar.float() + rays_d_lidar.float() * pred_depth.float().unsqueeze(-1)
            pred_lidar = torch.bmm(rays_d_lidar.float() * pred_depth.float().unsqueeze(-1), rotation) / self.opt.scale
            gt_lidar = torch.bmm(rays_d_lidar.float() * gt_depth.float().unsqueeze(-1), rotation) / self.opt.scale
        ...
        flow_time = field_flow_in_sensor(self.model, pred_world.float(), time_lidar, data["poses_lidar"])
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            ### dynamic feat
            f_idx = round(float(time_lidar.item()) * (self.opt.num_frames - 1))
        ...
        if self.opt.flow_loss:
            frame_idx = round(float(time_lidar.item()) * (self.opt.num_frames - 1))
    ...
    def eval_step(self, data):
        ...
        if self.opt.raydrop_loss == 'bce':
            pred_raydrop = F.sigmoid(pred_raydrop)
        if self.use_refine:
            inputs = refiner_input(outputs_lidar, H_lidar, W_lidar,
                                   probability_bce=self.opt.raydrop_loss == "bce")
            pred_raydrop = self.model.unet(inputs).squeeze(1)
    ...
    def train_update(self, data):
        # Retry an AMP overflow with the same batch and stochastic samples.
        # A failed backward must not consume an optimizer/scheduler update.
        rng = (random.getstate(), np.random.get_state(), torch.get_rng_state(),
               torch.cuda.get_rng_state(self.device) if self.fp16 else None)
        for attempt in range(9):
            if attempt:
                random.setstate(rng[0])
                np.random.set_state(rng[1])
                torch.set_rng_state(rng[2])
                torch.cuda.set_rng_state(rng[3], self.device)
            self.optimizer.zero_grad()
            with torch.cuda.amp.autocast(enabled=self.fp16):
                loss = self.train_step(data)[-1]
            if not torch.isfinite(loss):
                raise RuntimeError(f"non-finite loss at step {self.global_step}")
            self.scaler.scale(loss).backward()
            self.scaler.unscale_(self.optimizer)
            invalid = next((name for name, parameter in self.model.named_parameters()
                            if parameter.grad is not None
                            and not torch.isfinite(parameter.grad).all()), None)
            if invalid is None:
                self.scaler.step(self.optimizer)
                self.scaler.update()
                return loss.detach()
            if not self.scaler.is_enabled() or attempt == 8:
                raise RuntimeError(f"non-finite gradient in {invalid} at step {self.global_step}; "
                                   f"AMP retries exhausted ({attempt})")
            previous_scale = self.scaler.get_scale()
            # unscale_ recorded the overflow; update backs off the scale and
            # clears scaler state without ever applying the invalid gradients.
            self.scaler.update()
            self.log(f"[AMP] step {self.global_step}: overflow in {invalid}; "
                     f"scale {previous_scale:g} -> {self.scaler.get_scale():g}; "
                     f"retry same batch ({attempt + 1}/8)")
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            if self.global_step >= (self.opt.max_train_steps or self.opt.iters):
                break
            self.local_step += 1
            self.global_step += 1

            loss = self.train_update(data)
    ...
    def train(self, train_loader, valid_loader, refine_loader, max_epochs):
        # A completed run may be opened again using the documented resume
        # command. Reuse its terminal refiner instead of resetting it and
        # silently spending another refinement budget.
        if self.epoch >= max_epochs and not self.opt.skip_refine:
            refined_path = Path(self.ckpt_path) / f"{self.name}_ep{self.epoch:04d}_refine.pth"
            if refined_path.is_file():
                refined = torch.load(refined_path, map_location="cpu", weights_only=False)
                if refined.get("global_step") != self.global_step or "refine_contract" not in refined:
                    raise ValueError(f"refined checkpoint does not match field step {self.global_step}: {refined_path}")
                from model.visibility_refine import refinement_settings
                changed = [key for key, value in refinement_settings(self.opt).items()
                           if refined["refine_contract"].get(key) != value]
                if changed:
                    raise ValueError(
                        f"existing refinement uses different settings: {', '.join(changed)}. "
                        "Use --refine with an explicit field checkpoint and a new workspace "
                        "to run the requested refinement configuration."
                    )
                self.load_checkpoint(str(refined_path), model_only=True)
                self.ema = None  # refined weights are already materialized
                self.use_refine = True
                self.log(f"[INFO] Training and refinement already complete; loaded {refined_path}")
                return
        if self.use_tensorboardX:
            summary_path = os.path.join(self.workspace, "run", self.name)
            self.writer = tensorboardX.SummaryWriter(summary_path)

        if self.epoch < max_epochs and (self.opt.flow_loss or self.model_flow is not None):
    ...
    def refine(self, loader):
        from model.visibility_refine import refine
        refine(self, loader)
    ...
    def save_checkpoint(self, name=None, full=False, best=False, remove_old=True):
        ...
        if self.net_vlad is not None:
            state["net_vlad"] = self.net_vlad.state_dict()
    ...
    def load_checkpoint(self, checkpoint=None, model_only=False):
        if checkpoint is None:
            ...
            if self.field_training and not model_only:
                checkpoint_list = [path for path in checkpoint_list if not path.endswith("_refine.pth")]
            if checkpoint_list:
                checkpoint = checkpoint_list[-1]
                self.log(f"[INFO] Latest checkpoint is {checkpoint}")
            else:
                raise FileNotFoundError(f"no compatible checkpoint found in {self.ckpt_path}")

        checkpoint_dict = torch.load(checkpoint, map_location=self.device, weights_only=False)

        if self.field_training and not model_only:
            required = {"model", "optimizer", "lr_scheduler", "scaler", "epoch", "global_step"}
            if self.ema is not None:
                required.add("ema")
            if self.net_vlad is not None:
                required.add("net_vlad")
            missing = sorted(required - checkpoint_dict.keys())
            if "refine_contract" in checkpoint_dict or missing:
                raise ValueError(
                    f"{checkpoint} is not a complete field-training checkpoint; "
                    f"missing state: {', '.join(missing) or 'field stage'}. "
                    "Use --ckpt latest to resume field training, --ckpt latest_model "
                    "for fresh optimization from model weights, or --test_eval for evaluation."
                )

        model_state = checkpoint_dict.get("model", checkpoint_dict)
        ...
        if model_only:
            # The previous EMA describes the model before this load. A fresh
            # optimization must start its shadow from the loaded weights.
            if self.ema is not None:
                self.ema = ExponentialMovingAverage(self.model.parameters(), decay=self.ema_decay)
            return

        if self.field_training:
            if self.net_vlad is not None:
                self.net_vlad.load_state_dict(checkpoint_dict["net_vlad"], strict=True)
            if self.ema is not None:
                self.ema.load_state_dict(checkpoint_dict["ema"])
        else:
            # Standalone evaluation/refinement must use the same field weights
            # as the in-process path, which copies EMA before these stages.
            if "ema" in checkpoint_dict:
                evaluation_ema = ExponentialMovingAverage(self.model.parameters(), decay=self.opt.ema_decay)
                evaluation_ema.load_state_dict(checkpoint_dict["ema"])
                evaluation_ema.copy_to()
                self.log("[INFO] Materialized checkpoint EMA for evaluation/refinement.")
            self.ema = None
        ...
        if self.field_training:
            ...
            self.log("[INFO] Restored optimizer, scheduler and scaler.")
