...
from data.split_protocol import SPLIT_PROTOCOLS, transform_filename
...
class KITTI360Dataset(BaseDataset):
    ...
    def __post_init__(self):
        self.requested_split = self.split
        ...
        if self.split_protocol not in SPLIT_PROTOCOLS:
            raise ValueError(f"unknown split protocol {self.split_protocol!r}")
        self.transform_path = os.path.join(
            self.root_path,
            transform_filename(self.sequence_id, self.split, self.split_protocol),
        )
        ...
        for f in tqdm.tqdm(frames, desc=f"Loading {self.split} data"):
            ...
            self.frame_ids.append(int(f["frame_id"]))
        ...
        self.frame_ids = torch.tensor(self.frame_ids, dtype=torch.int64)
